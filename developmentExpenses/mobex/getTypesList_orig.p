...
for each bExpType no-lock
  ...
  assign
    cGroupType = "".
  if num-entries(bExpType.valdata) > 3 then cGroupType = (entry(4,  bExpType.valdata)).
  else next. /* ignore */
  ...
  where bExpGroup.valType = "ExpTypeGroup"
    ...
    message bExpType.valCode  "*" bExpType.description.
  /*  if bExpType.valCode3 > "" and bExpType.valCode3 <> "00" then do: /* company-specific? */
    if index(cCompList,bExpType.valCode3) = 0 then do:
      next LOOP_TYPES.
    end.
  end.
  message "OK made it - " bExpType.valDataType "*" bExpType.valCode "*"  bExpType.description. */
