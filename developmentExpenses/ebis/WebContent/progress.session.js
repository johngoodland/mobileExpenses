...
progress.data.Session = function Session( ) {
	...
	this._openRequest = function (xhr, verb, url, async) {
		...
		// Note that we do not pass the username and password. We rely on the browser
		// or native wrapper's credentials cache to supply them. This is to avoid a
		// problem in certain environments with PUT and DELETE requests when the service
		// is protected with Basic authentication.
		xhr.open(verb, urlPlusCCID, async);
